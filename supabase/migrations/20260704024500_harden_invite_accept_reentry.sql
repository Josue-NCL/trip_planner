...
begin
  ...
  select traveler.id
  into linked_traveler_id
  from public.trip_travelers traveler
  where traveler.trip_id = invite_row.trip_id
    and traveler.profile_id = current_profile_id
  order by traveler.sort_order, traveler.id
  limit 1;
  ...
  if exists (
    ...
    if invite_row.email = current_email and invite_row.status = 'pending' then
      update public.trip_invitations invitation
      set traveler_id = coalesce(linked_traveler_id, invitation.traveler_id),
          status = 'accepted',
          accepted_by = current_profile_id,
          accepted_at = coalesce(invitation.accepted_at, now())
      where invitation.id = invite_row.id;

      update public.trip_invitations invitation
      set status = 'revoked'
      where invitation.trip_id = invite_row.trip_id
        and invitation.email = invite_row.email
        and invitation.status = 'pending'
        and invitation.id <> invite_row.id;
    end if;
  ...
  if invite_row.traveler_id is null then
    ...
    from public.trip_travelers traveler
    where traveler.trip_id = invite_row.trip_id
      and traveler.client_id = 'invite-' || invite_row.id::text
      and (traveler.profile_id is null or traveler.profile_id = current_profile_id)
    limit 1;

    if linked_traveler_id is not null then
      ...
      while exists (
        ...
        from public.trip_travelers traveler
        where traveler.trip_id = invite_row.trip_id
          and lower(traveler.name) = lower(fallback_name)
      ) loop
        fallback_name := base_name || ' ' || name_suffix::text;
